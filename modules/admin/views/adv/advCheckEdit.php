<div id="page-inner">
    <div class="row">
        <div class="col-md-12">
            <h1 class="page-header">广告管理/ <small>修改待审核</small></h1>
        </div>
    </div>
    <!-- /. ROW  -->
    <div class="row">
    ...
    $("#doPass").click(function(){
        ...
        if(confirm("确定通过审核吗？")){
            $.ajax( {
                ...
                "dataType": "json",
                "data": {ids:ids,adv_status:7,status:"审核通过（修改）"}, //community_status审核状态，默认审核通过
                "success": function(data) {
                    console.log(data);
                    if(data > 0){
    ...
    $("#doFail").click(function(){
        ...
        if(confirm("确定审核驳回吗？")){
            $.ajax( {
                ...
                "url": "/admin/adv/docheck",
                "dataType": "json",
                "data": {ids:ids,adv_status:5,status:"审核驳回（修改）"}, //community_status审核状态，默认审核通过
                "success": function(data) {
                    console.log(data);
                    if(data > 0){
                        alert(data+"条记录状态修改成功！");
