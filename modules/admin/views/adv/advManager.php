...
function buildHtml(data){
    ...
    for(var key in data.list_data){
        ...
        html += '<tr><td><input type="checkbox" value="'+item.id+'" name="adv_id" />'+(parseInt(key)+1)+'</td><td>'+item.adv_no+'</td><td>'+item.adv_name+'</td><td>'+item.community_name+'</td><td>'+item.people_num+'</td><td>'+install_status[item.adv_install_status]+'</td><td>'+pic_status[item.adv_pic_status]+'</td><td>'+item.adv_rate+'</td><td>'+control_html+'</td></tr>';
    }
