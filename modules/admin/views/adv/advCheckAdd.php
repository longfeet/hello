    ...
    $("#doPass").click(function(){
        ...
        if(confirm("确定通过审核吗？")){
            $.ajax( {
                ...
                "dataType": "json",
                "data": {ids:ids,adv_status:7,status:"审核通过（新增）"}, //community_status审核状态，默认审核通过
                "success": function(data) {
                    console.log(data);
                    if(data > 0){
    ...
    $("#doFail").click(function(){
        ...
        if(confirm("确定审核驳回吗？")){
            $.ajax( {
                ...
                "url": "/admin/adv/docheck",
                "dataType": "json",
                "data": {ids:ids,adv_status:4,status:"审核驳回（新增）"}, //community_status审核状态，默认审核通过
                "success": function(data) {
                    console.log(data);
                    if(data > 0){
                        alert(data+"条记录状态修改成功！");
